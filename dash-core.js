...
function calcularDados(relatorio, backoffice) {
  ...
  // Grace — conta ativos por nível (1 a 4)
  const graceMap = { "Grace 1": 0, "Grace 2": 0, "Grace 3": 0};
let graceTotal = 0;
relatorio.filter(isAtivo).forEach(r => {
  const raw = String(r["Grace"] ?? "").trim();
  const v = raw.charAt(0).toUpperCase() + raw.slice(1).toLowerCase();
  if (graceMap[v] !== undefined) {
      graceMap[v]++;
      graceTotal++;
    }
  });
  const grace = { total: graceTotal, "Grace 1": graceMap["Grace 1"], "Grace 2": graceMap["Grace 2"], "Grace 3": graceMap["Grace 3"]};
console.log("Grace map:", JSON.stringify(graceMap), "Total:", graceTotal);
console.log("Exemplo Grace raw:", relatorio.filter(isAtivo).slice(0,5).map(r => JSON.stringify(r["Grace"])));
