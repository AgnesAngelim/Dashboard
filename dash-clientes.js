...
function renderizarClientes(d) {
  ...
  // Tabela: linhas ativadas e repetidas por cliente (top 100), com ID licenciado
...
tabDoc.innerHTML = c.docList
  ...
  .map(x => `
        ...
        <td>${x.repetidas > 0 ? `<span style="color:#F59E0B;font-weight:600;">${fmt(x.repetidas)}</span>` : `<span style="color:var(--text-muted);">0</span>`}</td>
      </tr>`).join("");
  }
  ...
  if (tabTels) tabTels.innerHTML = c.telsDetalhes.map(t=>`<tr><td style="font-family:monospace;font-size:12px;">${t.tel}</td><td><b>${fmt(t.count)}</b></td><td style="font-size:11px;color:var(--text-muted);">${(t.idsLicenciado||[]).slice(0,2).join(", ")}${(t.idsLicenciado||[]).length>2?` +${(t.idsLicenciado||[]).length-2}`:""}</td><td style="font-size:11px;color:var(--text-muted);">${t.docs.slice(0,3).join(", ")}${t.docs.length>3?` +${t.docs.length-3}`:""}</td></tr>`).join("");
